window.FrEMD = class {
	...
	async _getStoredScript(s) {
		if (this.sr()._) {
			let _sc = await this.sr()._("ContentManager.cmsStoredScriptFind", null, s);
			return _sc ? await sr.runScript(_sc.Script) : undefined;
		} else {
			console.log("_getStoredScript(): no sr() defined");
			return null;
		}
	}
...
<script>
	...
	async preInit() {
		...
		let oScope = this.__scope(company.Scope); //window[company.Scope];
	...
	async _loadEntityClasses(ec, scope = company.Scope) {
		...
		ret = await this._correctClasses(ec, eas);
		...
		(scope ? window[scope] : window).__script.Date = sr.serverDate();

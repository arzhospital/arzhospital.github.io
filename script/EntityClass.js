...
%>
	...
	this.EntityValues.push({EntityAttribute: {Id: <%=ea.Id%>, Name: "<%=ea.Name%>", EntityClass: {Id: <%=ea.EntityClass.Id%>}}});
...
%>
        if(this._<%=ta.Name.replace(" ", "_")%>_<%=taName%>_set){
            ret.ValueEntities.push(...this._<%=ta.Name.replace(" ", "_")%>_<%=taName%>.map(o => {
                return {Active: true, EntityAttribute: {Id: <%=ta.Id%>}, EntityObject: o.toEntityObject(bQuery)};
            }));
